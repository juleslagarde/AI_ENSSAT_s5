...
data_path = '/tmp/dataProjetAI/wikipedia_tabs.txt'
...
for line in lines[: min(num_samples, len(lines) - 1)]:
    ...
    # for the targets, and "\0" as "end sequence" character.
    target_text = '\t' + target_text[:max_text_length] + '\0'
...
fT = open("output.target", "w")
fP = open("output.predict", "w")
for seq_index in range(100):
    ...
    fT.write(target_texts[seq_index][:-1]+'\0')
    fP.write(decoded_sentence.replace('\0', '')+'\n')
